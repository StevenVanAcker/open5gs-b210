...
pushd base
docker image inspect docker_open5gs > /dev/null || docker build --force-rm -t docker_open5gs .
popd

pushd ims_base
docker image inspect docker_kamailio > /dev/null ||  docker build --force-rm -t docker_kamailio .
popd

pushd srslte
docker image inspect docker_srslte > /dev/null || docker build --force-rm -t docker_srslte .
popd
